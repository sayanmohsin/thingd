...
export type MemorydClusterMode = "single" | "leader" | "follower";
export type MemorydClusterDiscovery = "none" | "static" | "kubernetes";

export type MemorydClusterOptions = {
  mode?: MemorydClusterMode;
  advertiseUrl?: string;
  leaderUrl?: string;
  peers?: string[];
  discovery?: MemorydClusterDiscovery;
  service?: string;
  namespace?: string;
  port?: number;
...
export type ResolvedMemorydClusterOptions = {
  mode: MemorydClusterMode;
  advertiseUrl?: string;
  leaderUrl?: string;
  peers: string[];
...
export type MemorydClusterStatus = {
  mode: MemorydClusterMode;
  writable: boolean;
  forwarding: boolean;
  leaderUrl?: string;
...
export function readClusterOptionsFromEnv(
  ...
  return {
    mode: parseClusterMode(env.MEMORYD_CLUSTER_MODE),
    advertiseUrl: env.MEMORYD_ADVERTISE_URL,
    leaderUrl: env.MEMORYD_CLUSTER_LEADER_URL,
    peers: parsePeers(env.MEMORYD_CLUSTER_PEERS),
    discovery: parseClusterDiscovery(env.MEMORYD_CLUSTER_DISCOVERY),
    service: env.MEMORYD_CLUSTER_SERVICE,
    namespace: env.MEMORYD_CLUSTER_NAMESPACE,
    port: parsePort(env.MEMORYD_CLUSTER_PORT, DEFAULT_CLUSTER_PORT),
    forwardAuthToken: env.MEMORYD_CLUSTER_FORWARD_AUTH_TOKEN ?? env.MEMORYD_AUTH_TOKEN,
  };
}

export function resolveClusterOptions(
  options: MemorydClusterOptions | undefined,
  ...
  if (mode === "follower" && !options?.leaderUrl) {
    throw new Error("MEMORYD_CLUSTER_LEADER_URL is required when MEMORYD_CLUSTER_MODE=follower");
  }
...
export function clusterStatus(cluster: ResolvedMemorydClusterOptions): MemorydClusterStatus {
  return {
    mode: cluster.mode,
...
function parseClusterMode(value: string | undefined): MemorydClusterMode | undefined {
  if (!value) {
    return undefined;
  }
  ...
  throw new Error(`Unsupported MEMORYD_CLUSTER_MODE: ${value}`);
}

function parseClusterDiscovery(value: string | undefined): MemorydClusterDiscovery | undefined {
  if (!value) {
    return undefined;
  }
  ...
  throw new Error(`Unsupported MEMORYD_CLUSTER_DISCOVERY: ${value}`);
}
