...
|---|---|---|
| **MCP** | `POST /mcp` | Model Context Protocol — all 32 tools (search, put, get, delete, events, queues, links, aggregate, timeseries, schema, NLQ) |
...
thingd-server binary
  ...
  ├── mcp.rs           — MCP handler (all 32 tools via registry dispatch)
...
| MCP | All 32 tools native | All 32 tools via TypeScript |
