//! Local storage benchmark for memoryd-core adapters.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("memoryd storage benchmark");
    ...
    let database_path = directory.path().join("memoryd-bench.db");
    bench_store(
        "sqlite-file",
        SqliteMemoryStore::open(database_path)?,
        iterations,
    )?;
