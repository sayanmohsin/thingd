...
export function registerThingdTools(
  ...
  server.registerTool(
    ...
    async () => jsonResult(await db.listConnectors())
  );
  ...
  server.registerTool(
    "thing_connector_schema",
    {
      ...
      inputSchema: {
        ...
        query: z.string().min(1).describe("Table name (for DB connectors) or file path"),
      },
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
    ...
    async ({ type, auth, query }) => jsonResult(await db.discoverConnectorSchema(type, query, auth))
  );
  ...
  server.registerTool(
    "thing_connector_sync",
    {
      ...
      inputSchema: {
        ...
        auth: z
          ...
          .optional()
          .describe("Database credentials (for postgres/mysql)"),
        collection: z.string().min(1).describe("Target thingd collection name"),
        query: z.string().min(1).describe("SQL query or table name"),
        batchSize: z.number().int().positive().optional().describe("Rows per batch (default 1000)"),
        columnMapping: z
          .record(z.string(), z.string())
          .optional()
          .describe("Map external column names to thingd field names"),
        syncStrategy: z
          .enum(["full", "incremental"])
          .optional()
          .describe("Sync strategy (default: full)"),
      },
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
    ...
    async ({ type, auth, collection, query, batchSize, columnMapping, syncStrategy }) =>
      jsonResult(
        await db.connectorSync(type, {
          auth,
          collection,
          query,
          batchSize,
