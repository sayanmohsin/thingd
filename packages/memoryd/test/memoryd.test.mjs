...
const nativeBinaryPath = fileURLToPath(
  new URL("../../memoryd-native/dist/memoryd_native.node", import.meta.url),
);
const nativeAvailable = existsSync(nativeBinaryPath);

runMemoryDBehaviorSuite("memory", () => MemoryD.open(":memory:"));

if (nativeAvailable) {
  runMemoryDBehaviorSuite("native", () =>
    MemoryD.open({
      path: ":memory:",
      driver: "native",
    }),
  ...
  test("native: persists objects across reopen", async () => {
    const directory = await mkdtemp(join(tmpdir(), "memoryd-native-"));
    const path = join(directory, "memoryd.db");

    const first = await MemoryD.open({
      path,
      driver: "native",
    });
    await first.put("decisions", {
      id: "native-persistence",
      text: "Native memoryd writes to SQLite.",
    });

    const second = await MemoryD.open({
      path,
      driver: "native",
    });
    const stored = await second.get("decisions", "native-persistence");

    assert.equal(stored?.text, "Native memoryd writes to SQLite.");
...
function runMemoryDBehaviorSuite(label, openDb) {
