...
const dbPath = resolve("test-cli-c.db");
...
test("thingd snapshot create and restore works", async () => {
  ...
  // Verify snapshot contains the collections and events
  ...
  // 3. Mutate DB (delete user-1 and append a new event) to test restore
  const del = await run(["objects", "delete", "users", "user-1"]);
  assert.equal(del.code, 0);

  const getDeleted = await run(["objects", "get", "users", "user-1"]);
  assert.ok(getDeleted.stdout.includes("null") || getDeleted.code !== 0 || JSON.parse(getDeleted.stdout) === null);
