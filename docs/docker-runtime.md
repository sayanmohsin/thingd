...
Release CI and the local `pnpm smoke:docker` command validate the image
entrypoint, health, readiness, authenticated cluster status, and the MCP
tool-list handshake. Release CI installs the pinned Node and pnpm dependencies
before running the MCP check, so a missing client dependency fails validation
before publication.
