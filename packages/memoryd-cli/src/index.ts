...
const HELP_TEXT = `memoryd

Admin and operator CLI for memoryd.

Usage:
  memoryd status [--url <url>]
  memoryd tools --url <url>
  memoryd search <query> [--collection <name>] [--limit <n>]
  ...
  memoryd events list [stream] [--limit <n>]
  memoryd queues push <queue> --payload '{"key":"value"}'
  memoryd queues claim <queue> [--lease-ms <ms>]
  memoryd queues ack <queue> <jobId>
  memoryd queues nack <queue> <jobId> [--error <message>] [--delay-ms <ms>]
  memoryd queues list <queue> [--limit <n>]
  memoryd queues dead <queue> [--limit <n>]
...
async function runTools(context: CliContext): Promise<void> {
  ...
  if (!connection.remote) {
    throw new Error(
      "tools requires --url or MEMORYD_URL because tools are exposed by the MCP runtime",
    );
  }
...
async function withDb(
  context: CliContext,
  callback: (db: MemoryD) => Promise<void>,
): Promise<void> {
  const connection = resolveConnection(context);
...
function resolveConnection(context: CliContext): ConnectionOptions {
  const url = stringFlag(context.parsed, "url") ?? context.env.MEMORYD_URL;
  const path = url ?? stringFlag(context.parsed, "path") ?? context.env.MEMORYD_PATH ?? ":memory:";
  const remote = isRemotePath(path);
  const driver = parseDriver(stringFlag(context.parsed, "driver") ?? context.env.MEMORYD_DRIVER);

  return {
    path,
    driver: driver ?? (remote ? "remote" : undefined),
    authToken: stringFlag(context.parsed, "auth-token") ?? context.env.MEMORYD_AUTH_TOKEN,
    remote,
  };
}
...
function normalizeRemoteUrl(value: string): string {
  return value.startsWith("memoryd://") ? `http://${value.slice("memoryd://".length)}` : value;
}
